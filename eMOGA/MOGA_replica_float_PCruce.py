...
class eMOGA():
    ...
    Objfun_dim = 4  # Dimensiones del espacio objetivo
    searchSpace_dim =10  # Dimensiones espacio de busqueda
...
def Calcula_box(eMOGA,coste): #Funcion que determina en que caja debe estar cada dimension del vector de costes que recibe
    ...
    for i in range(eMOGA.Objfun_dim): #Analiza cada dimension del espacio objetivo
        if eMOGA.epsilon[i]==0: #Si el valor del ancho de caja (epsilon) es 0 determina que pertenece a la primera caja
            ...
            box[i]=M.ceil((coste[i]-eMOGA.min_f[i])/eMOGA.epsilon[i]) #Divide la distancia entre el coste y el limite inferior en regiones del ancho de caja (epsilon)
...
def evMOGA(eMOGA): #Recibe una instancia del algoritmo evMOGA y devuelve el frente de pareto, los elementos que lo componen y el propio eMOGA
    ...
    eMOGA.max_f = np.ones((eMOGA.Objfun_dim),dtype=float) #Genera un vector con los valores maximos para cada dimension sinedo estos tamaños 1 inicialmente
    eMOGA.min_f = np.ones((eMOGA.Objfun_dim),dtype=float)#Genera un vector con los valores minimos para cada dimension sinedo estos tamaños 1 inicialmente

    #Poblacion P
    eMOGA.ele_P=np.ones((eMOGA.Nind_P,eMOGA.searchSpace_dim),dtype=float)
    eMOGA.coste_P = np.ones((eMOGA.Nind_P, eMOGA.Objfun_dim),dtype=float)
    eMOGA.mod = np.zeros((eMOGA.Nind_P))

    # Polupation A
    eMOGA.ele_A = np.ones((1,eMOGA.searchSpace_dim),dtype=float)
    eMOGA.ele_A=np.delete(eMOGA.ele_A,0,axis=0)
    eMOGA.coste_A = np.ones((1,eMOGA.Objfun_dim),dtype=float)
    eMOGA.coste_A=np.delete(eMOGA.coste_A,0,axis=0)
    eMOGA.box_A = np.ones((1, eMOGA.Objfun_dim),dtype=float)
    eMOGA.box_A=np.delete(eMOGA.box_A,0,axis=0)
    eMOGA.Nind_A = 0

    # Polupation GA
    eMOGA.ele_GA = np.ones((eMOGA.Nind_GA,eMOGA.searchSpace_dim),dtype=float)
    eMOGA.coste_GA = np.ones((eMOGA.Nind_GA,eMOGA.Objfun_dim),dtype=float)
    eMOGA.box_GA = np.ones((eMOGA.Nind_GA, eMOGA.Objfun_dim),dtype=float)
    eMOGA.gen_counter = 0
