...
def Tamizado_datos(lines):
    ...
    if len(lines)>2:
        ...
        Porcentaje_sup_Pend,Media_Pend=Percentiles(Pendientes)
        Porcentaje_sup_Int,Media_Int=Percentiles(Intersecciones)


        if Porcentaje_sup_Pend>0:
            for line in lines:
...
def Crear_coordenadas(image,parametros_linea):
    ...
    if Pendiente>0.001 or Pendiente<-0.001:
        y1 = image.shape[0]
        y2 = int(y1 * (3 / 7))
...
def media_lineas(image,lines):
    ...
    Recta_Camara = [int((image.shape[1]/2)), int((image.shape[0]/2)+10), int((image.shape[1]/2)), int((image.shape[0]/2)-10)]
    ...
    if len(Grupo_total)>0:
        ...
        for Grupo in Grupo_total:
            ...
            if Interseccion_total * 1.1 > interseccion:
                Grupo_izquierda.append([Pendiente,interseccion])
            elif Interseccion_total * 0.9 < interseccion:
    ...
    return np.array([Linea_izquierda,Linea_derecha,Linea_central,Recta_Camara]),np.array([Linea_central])


#Recibe una imagen y un array de rectas y devuelve una imagen de las rectas sobre un fonde negro
def lines_detect(image, lines):
    lines_imagen = np.zeros_like(image)
    if lines is not None:
        for line in lines:
            x1,y1,x2,y2 = line.reshape(4)
            cv2.line(lines_imagen,(x1,y1),(x2,y2),(255,0,0),4)
    return lines_imagen
...
def canny(image,threshold):
    ...
    _, Umbral = cv2.threshold(gray, threshold, 240, cv2.THRESH_TOZERO)
    blur = cv2.GaussianBlur(Umbral,(5,5),0) # Se limpia de ruido la imagen aplicando un filtro de 5x5 a toda la imagen
    canny = cv2.Canny(blur,255,255)  # Se detectan gradientes importantes de color siendo el limite inferior 255 y el superior 255
    return canny
...
def Vision_artificial(result,lane_image):
    ...
    Lineas_medias, Linea_Central = media_lineas(lane_image, lineas)
    imagen_lines = lines_detect(lane_image, Lineas_medias)
...
def Vision_artificial_3(result):
    canny_imagen = canny(result, 210)
    lineas = cv2.HoughLinesP(canny_imagen, 2, np.pi / 180, 80, np.array([]), minLineLength=100, maxLineGap=3)
    Lineas_medias, Linea_Central = media_lineas(canny_imagen, lineas)
    imagen_lines = lines_detect(canny_imagen, Lineas_medias)
