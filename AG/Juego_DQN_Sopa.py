...
ciclo = 5  # Ciclos para los agentes random sin modificar su velocidad, de lo contrario se quedarian en el sitio practicamente
...
for i in range(simulacion.generations):
    ...
    for z in range(simulacion.steps):

        #Deciden acciones y actualizan su posicion
        ...
        for player, list_comida_ in colisiones.items():
            player.comida += sum(Comida.Alimento for Comida in
                                 list_comida_)  # Se suma la propiedad alimento de todas las comidas colisionadas a la comida del player
        ...
        Nueva_comida = rd.randint(0, (
                    simulacion.cantidad_comida - comida_presente))  # Aleatorio entre 0 y lo que falta por reponer
        ...
        #MEMORIZA Y ACTUALIZA NUEVO ESTADO
