...
class Player_basico(p.sprite.Sprite):
    ...
    def Digestion(self):  # 1.5 unidad de luz + 1 unidad de comida son 1.5 unidades de energia (Podrian hacerse digestiones mayores Ej:2 comida, 3 Luz --> 3 energia o multiplos asi si es posible)
    ...
    def Cagar(self,grupo): #Parte de la comida no debe ser util para el individuo por lo que se generará un residuo con un % del valor alimenticio de la comida
        mierda=Mierda()
    ...
    def Gasto_energia(self):
        Gasto = (abs(self.velocidad[0, 0]) + abs(self.velocidad[0, 1])) * 0.008  # Gasto de 0.3 de energia por pixel recorrido en cada direccion (¿Pitagoras para hacer la distancia?)
...
class player_IA(Player_basico): # Clase que a su vez instancia a la clase sprite de pygame
    def __init__(self):
        ...
        self.image = p.image.load("Imagenes/Cerebro.jfif")
    ...
    def Ver(self): # Extrae la imagen del entorno de cada agente
        ...
        y = self.rect.y # "
        mascara_imagen = image[y-100:y + 100, x-100:x + 100]  # Recortamos un rectangulo en la imagen con las cuatro esquinas como coordenadas
...
class simulacion():
    def __init__(self):
        self.generations = 5000 #Steps de la simulacion
